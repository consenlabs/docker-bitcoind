...

{
  ...
  node(step_name, commands, environment={},  version="latest", cache_path="", cache_name="node")::
    {
      name: step_name,
      image: 'node:' + version,
      commands: ['yarn config set cache-folder ' + cache(cache_name)] + commands,
    },
  ...
  publish(step_name, repo, when, tags=[], auto_tag=false, dockerfile="Dockerfile")::
    {
      ...
      settings: {
        ...
        repo: 'asia-docker.pkg.dev/imtoken-210003/imtoken/' + repo,
        tags: tags,
        cache_from: [
        "asia-docker.pkg.dev/imtoken-210003/imtoken/" + repo + ":" + "${DRONE_COMMIT_BEFORE:0:8}"
        ]
      },
      when: when,
    },
  ...
  deploy(step_name, env, namespace, repo, deployment, when, tag="${DRONE_COMMIT_SHA:0:8}"):: {
    name: step_name,
    image: 'asia-docker.pkg.dev/imtoken-210003/imtoken/drone-k8s',
    settings: {
